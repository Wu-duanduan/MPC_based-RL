...
def obstacle1(time_now, time_step):
    obs_ref = np.array([0,0,1.5],dtype=float)
    dic = {}
    obsCenter = np.array([obs_ref[0] + 0.6 * np.cos(0.5*time_now),
                          obs_ref[1] + 0.6 * np.sin(0.5*time_now),
                          obs_ref[2]], dtype=float)
    vObs = np.array([-0.3 * np.sin(0.5*time_now), 0.3 * np.cos(0.5*time_now), 0])

    time_now += time_step

    obsCenterNext = np.array([obs_ref[0] + 0.6 * np.cos(0.5*time_now),
                          obs_ref[1] + 0.6 * np.sin(0.5*time_now),
                          obs_ref[2]], dtype=float)
    vObsNext = np.array([-0.3 * np.sin(0.5*time_now), 0.3 * np.cos(0.5*time_now), 0])
    dic['v'] = vObs
