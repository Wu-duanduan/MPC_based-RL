...
class IIFDS:
    ...
    def __init__(self):
        ...
        self.cylinderH = 2
        self.uavR = 0.1
        self.start = np.array([-1.5,0,0.8],dtype=float)
        self.goal = np.array([1.5,0,0.8],dtype=float)
    ...
    def updateObs(self,if_test=False):
        ...
        if if_test:
            """测试环境"""
            self.timelog, dic = obs_list[0](self.timelog, self.timeStep)
        else:
            """否则用reset时随机的一个环境"""
    ...
    def calDynamicState(self, uavPos, obsCenter):
        """强化学习模型获得的state。"""
        s1 = (obsCenter - uavPos)*(self.distanceCost(obsCenter,uavPos)-(self.cylinderR+self.uavR))/self.distanceCost(obsCenter,uavPos)
        s2 = self.goal - uavPos
        s3 = self.vObs
    ...
    def getqNext(self, uavPos, obsCenter, vObs, row0, sigma0, theta, qBefore):
        ...
        repulsiveMatrix = self.calRepulsiveMatrix(uavPos, obsCenter, self.cylinderR + self.uavR, row0)
        tangentialMatrix = self.calTangentialMatrix(uavPos, obsCenter, self.cylinderR + self.uavR, theta, sigma0)
        T = self.calculateT(obsCenter, uavPos, self.cylinderR + self.uavR)
