...
class DDPG:
    def __init__(self, obs_dim, act_dim, num, actor_critic=core.MLPActorCritic,
                replay_size=int(1e5), gamma=0.99, polyak=0.995, pi_lr=1e-3, q_lr=1e-3, sr_lr=1e-3, act_noise=0.1):
        ...
        # self.sr_net = SR(obs_dim, act_dim, num).to(device)
        # self.sr_optimizer = Adam(self.sr_net.parameters(), lr=LR_SR, weight_decay=WEIGHT_DECAY)
        self.sr_optimizer = Adam(self.ac.sr.parameters(), lr=sr_lr)
    ...
    def compute_loss_q(self, data):   #返回(q网络loss, q网络输出的状态动作值即Q值)
        ...
        o1_values = self.ac.sr(o0, a0)[:, 0:self.obs_dim]
        r0_values = self.ac.sr(o0, a0)[:, self.obs_dim]  # R值

        a1_values = self.ac.pi(o1_values)

        o2_values = self.ac.sr(o1_values, a1_values)[:, 0:self.obs_dim]
        r1_values = self.ac.sr(o1_values, a1_values)[:, self.obs_dim]  # R+1值
        ...
        loss_sr = ((o1 - o1_values)**2).mean() + ((r0 - r0_values)**2).mean()

        return loss_q, loss_sr # 这里的loss_q没加负号说明是最小化，很好理解，TD正是用函数逼近器去逼近backup，误差自然越小越好
    ...
    def update(self, data):
        ...
        loss_q, loss_sr = self.compute_loss_q(data)
