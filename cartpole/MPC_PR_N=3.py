...
class DQN:
    ...
    def update(self, transition_dict):
        ...
        q2_values = self.q_net(s2_values).gather(1, a2_values)  # Q+1值
        s3_1_values = torch.reshape(self.sr_net(s2_values, a2_values)[:, 0], [batch_size, 1])  # S+2值
        s3_2_values = torch.reshape(self.sr_net(s2_values, a2_values)[:, 1], [batch_size, 1])  # S+2值
        s3_3_values = torch.reshape(self.sr_net(s2_values, a2_values)[:, 2], [batch_size, 1])  # S+2值
        s3_4_values = torch.reshape(self.sr_net(s2_values, a2_values)[:, 3], [batch_size, 1])  # S+2值
        s3_values = torch.cat((s3_1_values, s3_2_values, s3_3_values, s3_4_values), 1)
        r2_values = torch.reshape(self.sr_net(s2_values, a2_values)[:, 4], [batch_size, 1])  # R+1值
        # 下个状态的最大Q值
...
# savemat('DQN_MPC4.mat', {'episodes_list': episodes_list, 'return_list': return_list, 'mv_return': mv_return, 'sr_loss': sr_loss_list})
np.save('mv_return44.npy', mv_return)
np.save('sr_loss44.npy', sr_loss_list)
