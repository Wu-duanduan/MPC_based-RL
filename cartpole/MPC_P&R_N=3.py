...
class DQN:
    ...
    def __init__(self, state_dim, hidden_dim, action_dim, learning_rate, gamma,
        ...
        # 模型网络
        self.s_net = S_net(state_dim, hidden_dim,
                           self.action_dim).to(device)  # S+1网络
        self.r_net = Rnet(state_dim, hidden_dim,
                          self.action_dim).to(device)  # R网络
        # 使用Adam优化器
        self.optimizer1 = torch.optim.Adam(self.q_net.parameters(),
                                           lr=learning_rate)
        self.optimizer2 = torch.optim.Adam(self.s_net.parameters(),
                                           lr=learning_rate)
        self.optimizer3 = torch.optim.Adam(self.r_net.parameters(),
                                           lr=learning_rate)
    ...
    def update(self, transition_dict):
        ...
        s1_4_values = torch.reshape(self.s_net(states, actions)[:, 3], [batch_size, 1])  # S+1值
        ...
        s2_4_values = torch.reshape(self.s_net(s1_values, a1_values)[:, 3], [batch_size, 1])  # S+2值
        s2_values = torch.cat((s2_1_values, s2_2_values, s2_3_values, s2_4_values), 1)
        r1_values = self.r_net(s1_values, a1_values)  # R+1值

        a2_values = self.q_net(s2_values).argmax(dim=1)
        a2_values = torch.reshape(a2_values, [batch_size, 1])

        q2_values = self.q_net(s1_values).gather(1, a1_values)  # Q+1值
        ...
        s3_4_values = torch.reshape(self.s_net(s2_values, a2_values)[:, 3], [batch_size, 1])  # S+2值
        s3_values = torch.cat((s3_1_values, s3_2_values, s3_3_values, s3_4_values), 1)
        ...
        max_next_q3_values = self.target_q_net(s3_values).max(1)[0].view(
            -1, 1)
        q2_targets = r2_values + self.gamma * max_next_q3_values * (1 - dones
                                                                    )  # TD误差目标
        dqn1_loss = torch.mean(F.mse_loss(q0_values, q0_targets) + F.mse_loss(q1_values, q1_targets)
        ...
        for i_episode in range(int(num_episodes / 20)):
            ...
            while not done:
                ...
                if replay_buffer.size() > minimal_size:
                    ...
                    dqn2_loss, dqn3_loss = agent.update(transition_dict)
                    s_loss += dqn2_loss
                    r_loss += dqn3_loss
...
# savemat('DQN_MPC3.mat', {'episodes_list': episodes_list, 'return_list': return_list, 'mv_return': mv_return, 's_loss': s_loss_list, 'r_loss': r_loss_list})
plt.plot(episodes_list, mv_return)
np.save('mv_return73.npy', mv_return)
np.save('s_loss33.npy', s_loss_list)
np.save('r_loss33.npy', r_loss_list)
plt.xlabel('Episodes')
plt.ylabel('Returns')
plt.title('DQN on {}'.format(env_name))
plt.show()